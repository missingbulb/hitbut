...
4. **Record it in the pack** — a row at the top of the owning pack's `VERSIONS.md` table per conversion, in the same commit: date, `prose-to-checks-sweep`, the prose converted and the check id it became. That file is the record; there is no standing issue. A run that converted nothing writes no row.
